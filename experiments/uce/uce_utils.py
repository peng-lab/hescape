...
class AnndataProcessor:
    def __init__(self, args, accelerator):
        ...
        self.h5_folder_path = self.args.temp_dir
        self.npz_folder_path = self.args.temp_dir
        self.scp = ""
        ...
        self.pe_idx_path = self.args.temp_dir + "/" + f"{self.name}_pe_idx.torch"
        self.chroms_path = self.args.temp_dir + "/" + f"{self.name}_chroms.pkl"
        self.starts_path = self.args.temp_dir + "/" + f"{self.name}_starts.pkl"
        self.shapes_dict_path = self.args.temp_dir + "/" + f"{self.name}_shapes_dict.pkl"
    ...
    def generate_idxs(self):
        if self.accelerator.is_main_process:
            if (
                os.path.exists(self.pe_idx_path)
                and os.path.exists(self.chroms_path)
                and os.path.exists(self.starts_path)
            ):
...
def run_eval(adata, name, pe_idx_path, chroms_path, starts_path, shapes_dict, accelerator, args):
    ...
    dataloader = DataLoader(
        dataset, batch_size=batch_size, shuffle=False, collate_fn=multi_dataset_sentence_collator, num_workers=8
    )
    ...
    with torch.no_grad():
        # i = 0
        for batch in pbar:
            batch_sentences, mask, _ = batch[0], batch[1], batch[2]
            ...
            if accelerator.is_main_process:
                dataset_embeds.append(embeddings.cpu().numpy())

            # Cleanup tensors
            del embeddings, embedding, batch_sentences
    ...
    if accelerator.is_main_process:
        ...
        write_path = args.final_dir + "/" + f"{name}_uce_adata.h5ad"
        adata.write(write_path)

        print("*****Wrote Anndata to:*****")
        print(write_path)

    del adata, model, dataloader, dataset_embeds
    gc.collect()
    torch.cuda.empty_cache()
