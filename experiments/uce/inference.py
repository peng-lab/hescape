...
def main(args, save_root_dir):
    accelerator = Accelerator(project_dir=save_root_dir)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Embed a single anndata using UCE.")

    # Anndata Processing Arguments
    # Updated Arguments
    parser.add_argument(
        "--adata_dir", type=str, default=None, help="Directory containing the anndatas you want to embed."
    )
    parser.add_argument("--temp_dir", type=str, default="./", help="Working folder where all temp files will be saved.")
    parser.add_argument("--final_dir", type=str, default="./", help="Working folder where final adata will be saved.")
    parser.add_argument("--species", type=str, default="human", help="Species of the anndata.")
    parser.add_argument("--filter", type=bool, default=True, help="Additional gene/cell filtering on the anndata.")
    parser.add_argument(
        "--skip", type=bool, default=True, help="Skip datasets that appear to have already been created."
    )
    ...
    parser.add_argument("--CXG", type=bool, default=True, help="Use CXG model.")
    ...
    parser.add_argument(
        "--spec_chrom_csv_path",
        default="./model_files/species_chrom.csv",
        type=str,
        help="CSV Path for species genes to chromosomes and start locations.",
    )
    parser.add_argument(
        "--token_file", default="./model_files/all_tokens.torch", type=str, help="Path for token embeddings."
    )
    parser.add_argument(
        "--protein_embeddings_dir",
        default="./model_files/protein_embeddings/",
        type=str,
        help="Directory where protein embedding .pt files are stored.",
    )
    parser.add_argument(
        "--offset_pkl_path",
        default="./model_files/species_offsets.pkl",
        type=str,
    ...
    anndata_files = glob.glob(os.path.join(args.adata_dir, "*.h5ad"))
    if not anndata_files:
        print(f"No .h5ad files found in directory {args.adata_dir}")
    else:
        for anndata_file in anndata_files:
            try:
                dataset_name = os.path.splitext(os.path.basename(anndata_file))[0]
                output_temp_subdir = os.path.join(temp_dir, dataset_name)

                # Create subdirectory for the dataset
                os.makedirs(output_temp_subdir, exist_ok=True)

                # Update args for current anndata
                args.adata_path = anndata_file
                args.temp_dir = output_temp_subdir

                print(f"Processing {anndata_file}...")
                main(args, temp_dir)

                # Explicitly delete all references to GPU tensors
                gc.collect()
                torch.cuda.empty_cache()
                print(f"Completed processing {anndata_file}")
                print(f"GPU memory cleared for {anndata_file}")

                # delete the args.temp_dir folder and recursive contents
                # shutil.rmtree(temp_dir)

            except Exception as e:
                raise Exception(f"Error processing {anndata_file}: {e}") from e
